...
public class Cycle2DDN : MonoBehaviour {
	...
	public float cycleUpdate;

	/// <summary>
	/// The lists to hold all the renderers.
	/// </summary>
	List<SpriteRenderer> AnimatedSprites, StaticSprites;
	public List<Renderer> MiscRenderers;	// Uncomment this and line 147 - 151 and use this list for misc mesh renderers.
	...
	[HideInInspector] public Color dayT, duskT, nightT, dawnT;
    ...
    private void Awake()
    {
        if(staticBool)
        {
            ...
            startingCycle = scene.cycleTime;
            cycle = scene.cycleTime;
            if (overworld.spawnedOW == true)
            {
                MiscRenderers = overworld.MiscRenderers;
                //  staticBool = true;
            }
            else
            {
                overworld.MiscRenderers = MiscRenderers;
                staticBool = true;
            }
            t = scene.storedTOW;
    ...
    void UpdateMainColor () {
		...
		case 3:
		//	mainColor = Color.Lerp (dawn, dawn, t);
    ...
    void UpdateStatic2 () {
		...
		screenDark.color = scene.scrnColor;
	}
   ...
   /* public void JumpToCycle2 (int selCycle) {
        Debug.Log("Jump2");
		// Error incase the selected cycle doesn't exist.
		if (selCycle < 0 || selCycle > 3) {
			Debug.LogError ("WHOOPSIE DAISY! Looks like you tried to make the day night handler jump to a cycle that doesn't exist." +
        ...
        switch (cycle)
        {
            ...
            case 0:
                mainColor = scene.dayT;
                scrnColor = scene.scrnColor;
                break;

            case 1:
                mainColor = scene.duskT;
                scrnColor = scene.scrnColor;
                break;

            case 2:
                mainColor = scene.nightT;
                Debug.Log("night");
                scrnColor = scene.scrnColor;
                break;

            case 3:
                mainColor = scene.dawnT;
                scrnColor = scene.scrnColor;
                break;
        }
